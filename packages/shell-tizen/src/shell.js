...
(function () {
    ...
    try { if (!window.__shellT0) window.__shellT0 = Date.now(); } catch (_) {}
    ...
    var MODERN_SYNTAX_RE_SRC = '\\?\\.|\\?\\?|\\?\\?=|\\|\\|=|&&=|(^|[^\\w])#[a-zA-Z_$]|\\d_\\d|\\d+n\\b';
    ...
    var BABEL_OPTS_KEY = 'presets:[[env,{targets:{chrome:56},modules:false}]];sourceType:script;compact:true;comments:false';
    ...
    var TX_VER = txFnv1a(MODERN_SYNTAX_RE_SRC + '|' + BABEL_OPTS_KEY + '|' + BABEL_FPR);
    var TX_PFX = 'shell.tx' + TX_VER + ':';
    try { window.__TXVER = TX_VER; } catch (_) {}
    ...
    function writeBundlePatchState(state) {
        var rec = { v: BUNDLE_CACHE_VER, url: state.url, needsPatch: !!state.needsPatch };
        ...
        if (state.body && state.body.length <= MAIN_BUNDLE_BODY_MAX) {
            rec.body = state.body;
            if (state.needsPatch && typeof state.patches === 'number') rec.patches = state.patches;
        }
        try {
            localStorage.setItem(BUNDLE_CACHE_KEY, JSON.stringify(rec));
            return;
        } catch (_) {
            try { window.__shellMainBundleQuotaErr = 1; } catch (__) {}
        }
        ...
        if (rec.body) {
            delete rec.body;
            delete rec.patches;
            try { localStorage.setItem(BUNDLE_CACHE_KEY, JSON.stringify(rec)); } catch (__) {}
        }
    }
    ...
    function webCacheEnabled() {
        try { return localStorage.getItem(WEB_CACHE_GATE_KEY) === '1'; } catch (_) { return false; }
    }
    ...
    function writeWebIndexCache(serverOrigin, body) {
        ...
        var rec = { v: WEB_CACHE_VER, origin: serverOrigin, ts: Date.now(), size: body.length, body: body };
        try { localStorage.setItem(WEB_INDEX_CACHE_KEY, JSON.stringify(rec)); } catch (_) {}
    }
    ...
    function readWebConfigCache(serverOrigin) {
        try {
            ...
            try { p.parsed = JSON.parse(p.body); } catch (_) { return null; }
            return p;
        } catch (_) { return null; }
    }
    ...
    function writeWebConfigCache(serverOrigin, bodyText) {
        ...
        try { JSON.parse(bodyText); } catch (_) { return; }
        var rec = { v: WEB_CACHE_VER, origin: serverOrigin, ts: Date.now(), size: bodyText.length, body: bodyText };
        try { localStorage.setItem(WEB_CONFIG_CACHE_KEY, JSON.stringify(rec)); } catch (_) {}
    }
    ...
    function registerRemoteKeys() {
        ...
        keys.forEach(function (k) {
            try { tizen.tvinputdevice.registerKey(k); } catch (e) { /* not fatal */ }
        });
    }
    ...
    function exitApp() {
        if (hasTizen && tizen.application) {
            try { tizen.application.getCurrentApplication().exit(); return; } catch (e) { /* fallthrough */ }
        }
        window.close();
    }
    ...
    function generateDeviceId() {
        return btoa([navigator.userAgent, Date.now(), Math.random()].join('|')).replace(/=/g, '1');
    }
            ...
            tizen.systeminfo.getPropertyValue('DISPLAY', function (result) {
                ...
                try {
                    if (hasWebapis && webapis.productinfo) {
                        if (typeof webapis.productinfo.is8KPanelSupported === 'function' && webapis.productinfo.is8KPanelSupported()) ratio = 4;
                        else if (typeof webapis.productinfo.isUdPanelSupported === 'function' && webapis.productinfo.isUdPanelSupported()) ratio = 2;
                    }
                } catch (e) { /* ignore */ }
                systeminfo = {
    ...
    var SupportedFeatures = [
        'exit', 'exitmenu', 'externallinkdisplay',
        'htmlaudioautoplay', 'htmlvideoautoplay',
        'physicalvolumecontrol', 'displaylanguage',
        'otherapppromotions', 'targetblank', 'screensaver',
        'multiserver', 'subtitleappearancesettings', 'subtitleburnsettings'
    ];

    window.NativeShell = {
        AppHost: {
            init: function () {
                return getSystemInfo().then(function () { return AppInfo; });
            },
            ...
            screen: function () {
                return systeminfo
                    ? { width: systeminfo.resolutionWidth, height: systeminfo.resolutionHeight }
                    : null;
            },
        ...
        enableFullscreen: function () { /* no-op: WebView is always fullscreen on TV */ },
        disableFullscreen: function () { /* no-op */ },
        openUrl: function (/* url, target */) { /* TV cannot open external browsers */ },
        updateMediaSession: function () { /* no native media session yet */ },
        hideMediaSession: function () { /* no native media session yet */ },
        getPlugins: function () { return []; },
        downloadFile: function () { /* offline downloads not in M1 */ },
    ...
    try { SHELL_DEBUG = localStorage.getItem('jellyfin.shell.debug') === '1'; } catch (e) { /* ignore */ }
    function shellLog() {
    ...
    function babelTranspile(src) {
        try {
            ...
            try { console.warn('shell: babel transpile failed', e && e.message); } catch (_) {}
            return null;
        }
    }
    ...
    function buildDiagSeedScript(shellVersion) {
        return [
            ...
            '  var lines=["shell v' + shellVersion + ' legacy="+(init.legacy?"1":"0")+" babel="+(init.babel?"1":"0")+" poly="+(init.polyfilled?"1":"0"),',
    ...
    function injectChromium56Polyfills(doc) {
        ...
        if (seedTag && seedTag.nextSibling) doc.head.insertBefore(polyfillTag, seedTag.nextSibling);
    ...
    function txGetStatic(url) {
        try {
            ...
            if (v == null) {
                ...
                if (!miss) { miss = []; window.__shellTxCacheMissUrlsStatic = miss; }
    ...
    function txSetStatic(url, body) {
        ...
        try { localStorage.setItem(TX_PFX + txKey(url), body); } catch (_) { /* quota — soft fail */ }
    }
    ...
    function transpileLegacyScripts(doc, baseUrl) {
        ...
        try {
            var prevStreakInit = parseInt(localStorage.getItem(BABEL_UNUSED_STREAK_KEY) || '0', 10) || 0;
            window.__shellBabelUnusedStreak = prevStreakInit;
        } catch (_) { window.__shellBabelUnusedStreak = window.__shellBabelUnusedStreak || 0; }
        return transpileLegacyScriptsInner(doc, baseUrl).then(function (r) {
            try {
                var c = window.__shellDiagInit || {};
                var prev = 0;
                try { prev = parseInt(localStorage.getItem(BABEL_UNUSED_STREAK_KEY) || '0', 10) || 0; } catch (_) {}
                var next = prev;
                if ((c.scriptsFound || 0) > 0) {
                    if ((c.babelLazyTriggered || 0) === 0 && (c.cachedHits || 0) === c.scriptsFound) {
                        next = prev + 1;
                    } else {
                        next = 0;
                    }
                    try { localStorage.setItem(BABEL_UNUSED_STREAK_KEY, String(next)); } catch (_) {}
                }
    ...
    function ensureBabelReady() {
        var ensure = (typeof window.__ensureBabel === 'function')
            ? window.__ensureBabel
            : function () { return Promise.resolve(); };
        var p;
        try { p = ensure(); } catch (_) { p = Promise.resolve(); }
        if (!p || typeof p.then !== 'function') p = Promise.resolve();
        return p.then(function () {
            var ok = typeof window.Babel !== 'undefined';
            if (ok) {
                try { localStorage.setItem(BABEL_NEEDED_KEY, '1'); } catch (_) {}
            }
            return ok;
        });
    }
    ...
    function transpileLegacyScriptsInner(doc, baseUrl) {
        var legacy = isLegacyChromium();
        shellLog('transpile gate: legacy=' + legacy + ' babel(initial)=' + (typeof window.Babel !== 'undefined'));
        ...
        for (var pUi = 0; pUi < scripts.length && pluginUrlsForNextBoot.length < 100; pUi++) {
            var pUs = scripts[pUi];
        ...
        try { localStorage.setItem('jellyfin.shell.pluginUrls', JSON.stringify(pluginUrlsForNextBoot)); } catch (_) {}
        ...
        try { sbServerOrigin = new URL(baseUrl).origin; } catch (_) {}
        var secondaryBundleUrls = [];
        var sbSeen = {};
        var SB_MAIN_RE = /(?:^|\/)main\.[^/]*\.bundle\.js$/i;
        for (var bUi = 0; bUi < scripts.length && secondaryBundleUrls.length < 20; bUi++) {
            var bUs = scripts[bUi];
            ...
            var bUurl;
            try { bUurl = new URL(bUsrc, baseUrl).href; } catch (_) { continue; }
            if (sbServerOrigin) {
                var bUorigin;
                try { bUorigin = new URL(bUurl).origin; } catch (_) { continue; }
        ...
        try { localStorage.setItem('jellyfin.shell.secondaryBundleUrls', JSON.stringify(secondaryBundleUrls)); } catch (_) {}
        ...
        var stylesheetUrls = [];
        var ssSeen = {};
        for (var lUi = 0; lUi < ssLinks.length && stylesheetUrls.length < 20; lUi++) {
            var lUh = ssLinks[lUi].getAttribute('href');
            if (!lUh) continue;
            if (/^(?:data|blob|javascript):/i.test(lUh)) continue;
            var lUurl;
            try { lUurl = new URL(lUh, baseUrl).href; } catch (_) { continue; }
            if (sbServerOrigin) {
                var lUorigin;
                try { lUorigin = new URL(lUurl).origin; } catch (_) { continue; }
        ...
        try { localStorage.setItem('jellyfin.shell.stylesheetUrls', JSON.stringify(stylesheetUrls)); } catch (_) {}
        var jobs = scripts.map(function (s) {
            ...
            if (s.getAttribute('data-shell-bundle-patched')) { counts.skipped++; return null; }
            var src = s.getAttribute('src');
            if (src) {
                if (isJellyfinWebBundle(src)) { counts.skipped++; return null; }
                counts.scriptsFound++;
                var url;
                try { url = new URL(src, baseUrl).href; } catch (_) { return null; }
                ...
                return responsePromise
                    ...
                    .then(function (code) {
                        ...
                        return ensureBabelReady().then(function (ready) {
                            if (!ready) {
                                ...
                                try { console.warn('shell: babel not available, skip transpile', url); } catch (_) {}
                                return;
                            }
                            counts.babel = true;
                            var out = babelTranspile(code);
                            if (out == null) { counts.transpileFailed++; return; }
                            counts.transpiled++;
                    ...
                    .catch(function (e) {
                        counts.transpileFailed++;
                        try { console.warn('shell: skip transpile', url, e && e.message); } catch (_) {}
                    });
            }
            ...
            if (!needsTranspile(content)) { counts.fastPath++; return null; }
            // JEL-1034 (v53): slow path triggers lazy babel load.
            counts.babelLazyTriggered++;
            return ensureBabelReady().then(function (ready) {
                if (!ready) {
                    try { console.warn('shell: babel not available, skip inline transpile'); } catch (_) {}
                    return;
                }
                counts.babel = true;
    ...
    function buildBundleSourcePatcher() {
        ...
        return function patch(source) {
            ...
            for (var p = 0; p < patterns.length; p++) {
                source = source.replace(patterns[p], function (_match, prefix, paramName) {
                    total++;
                    return prefix +
                        'try{' +
                            'if(' + paramName + '==null&&window.ApiClient)return window.ApiClient;' +
                            'if(' + paramName + '&&typeof ' + paramName + '==="object"&&!' + paramName + '.ServerId&&window.ApiClient&&typeof window.ApiClient.serverId==="function")' +
                                paramName + '.ServerId=window.ApiClient.serverId();' +
                        '}catch(_){}' +
                        'if(!' + paramName + ')throw new Error("item or serverId cannot be null")';
                });
            }
    ...
    function patchPlaybackBundles(doc, baseUrl, prefetched) {
        ...
        var jobs = scripts.map(function (s) {
            ...
            if (!/(^|\/)main\.[^/]*\.bundle\.js$/i.test(bare) && !/(^|\/)main\.jellyfin\.bundle\.js$/i.test(bare)) return null;
            var url;
            try { url = new URL(src, baseUrl).href; } catch (_) { return null; }
            ...
            if (cache && cache.url === url) {
                if (cache.body && cache.body.indexOf('</script') < 0) {
                    ...
                    var cachedPatches = (cache.needsPatch && typeof cache.patches === 'number' && cache.patches > 0) ? cache.patches : 0;
                    ...
                    window.__shellBundlesPatchedFiles.push(bare.split('/').pop() + ':cache' + cachedPatches);
                    window.__shellBundleCacheHit++;
                    window.__shellBundleCacheBodyHit++;
                    window.__shellMainBundleLSAdopted = 1;
                    window.__shellMainBundleInlineHits = (window.__shellMainBundleInlineHits || 0) + 1;
            ...
            return bundleFetch
                .then(function (r) {
                    ...
                    try { localStorage.setItem('jellyfin.shell.bundleUrl', url); } catch (_) {}
                    return r.text();
                })
                .then(function (code) {
                    ...
                    if (result.patches === 0) {
                        try { console.warn('shell: bundle has error string but no pattern matched', url); } catch (_) {}
                        return;
                    }
                    s.removeAttribute('src');
                    s.removeAttribute('defer');
                    ...
                    window.__shellBundlesPatchedFiles.push(bare.split('/').pop() + ':' + result.patches);
                    ...
                    writeBundlePatchState({ url: url, needsPatch: true, body: result.source, patches: result.patches });
                    try { console.log('shell: patched bundle', url, 'patches=' + result.patches); } catch (_) {}
                })
                .catch(function (e) {
                    try { console.warn('shell: bundle patch fetch failed', url, e && e.message); } catch (_) {}
                });
        });
        return Promise.all(jobs);
    }
    ...
    function armDeferWatchdog() {
        var POLL = 150, CAP = 5500, started = Date.now(), hangHits = 0;
        function reinject(reason) {
            try {
                ...
                try { console.warn('shell: defer-script watchdog firing (' + reason + '); re-injecting', defers.length, 'scripts'); } catch (_) {}
                ...
                try { console.warn('shell: defer-script watchdog error', e && e.message); } catch (_) {}
            }
        }
        function tick() {
            try {
                ...
                if (document.readyState === 'complete') {
                    hangHits++;
                    if (hangHits >= 2) { reinject('readyState-complete@' + elapsed + 'ms'); return; }
                } else {
                    hangHits = 0;
                }
                if (elapsed >= CAP) { reinject('cap@' + elapsed + 'ms'); return; }
                setTimeout(tick, POLL);
            } catch (e) {
                try { console.warn('shell: defer-script watchdog tick error', e && e.message); } catch (_) {}
            }
        }
        setTimeout(tick, POLL);
    }
    ...
    var BUNDLE_FAST_RE = /<script\b[^>]*\bsrc\s*=\s*["']([^"']*main\.[^"']*\.bundle\.js[^"']*)["'][^>]*>\s*<\/script>/i;
    function maybeStringFastPath(html, serverUrl, baseUrl, upstreamCfg) {
        ...
        try { if (localStorage.getItem('jellyfin.shell.fastPathDisabled') === '1') return bail('killSwitch'); } catch (_) {}
        ...
        try { babelNeeded = localStorage.getItem(BABEL_NEEDED_KEY) === '1'; } catch (_) {}
        ...
        var cachedPatches = 0;
        if (bundleMatch) {
            try { bundleUrl = new URL(bundleMatch[1], baseUrl).href; } catch (_) { return bail('bundleUrlParse'); }
            var cache = readBundlePatchState();
            ...
            if (cache.body) {
                ...
                cachedPatches = (cache.needsPatch && typeof cache.patches === 'number' && cache.patches > 0) ? cache.patches : 0;
                ...
                if (inlineBundleBody.indexOf('</script') >= 0) return bail('bundleScriptClose');
        ...
        var beaconTag = (beaconBody && beaconBody !== '__QA_BEACON_BODY__')
            ? '<script data-shell-beacon="1">' + beaconBody + '</script>'
            : '';
        var injected =
            '<script data-shell-diag="1">' + diagBody + '</script>' +
            '<base href="' + escAttr(baseUrl) + '">' +
            '<script data-shell-seed="1">' + seedBody + '</script>' +
            '<script data-shell-polyfill="1">' + polyBody + '</script>' +
            beaconTag;
        ...
        if (inlineBundleBody) {
            ...
            patched = patched.replace(BUNDLE_FAST_RE, function (m) {
                ...
                replaced = true;
                return '<script data-shell-bundle-patched="' + escAttr(bundleUrl) +
                    '" data-shell-bundle-from-cache="1" data-shell-bundle-patches="' + cachedPatches +
                    '">' + inlineBundleBody + '</script>';
            });
    ...
    function markDocumentWrite() {
        try {
            ...
            else localStorage.setItem('jellyfin.qa.bootMarks.current', JSON.stringify(window.__qaMarks));
        } catch (_) {}
    }
    ...
    function loadRemoteWebClient(serverUrl) {
        ...
        try { babelNeededFlag = localStorage.getItem(BABEL_NEEDED_KEY) === '1'; } catch (_) {}
        ...
        try { babelStreakSkip = (parseInt(localStorage.getItem(BABEL_UNUSED_STREAK_KEY) || '0', 10) || 0) >= 2; } catch (_) {}
        window.__shellBabelPrimeSkipped = (babelStreakSkip ? 1 : 0);
        if (isLegacyChromium() && babelNeededFlag && !babelStreakSkip && typeof window.__ensureBabel === 'function') {
            try { window.__ensureBabel(); } catch (_) {}
        }
        ...
        var indexFetch = (pf && pf.baseUrl === baseUrl && pf.index) ? pf.index : fetch(baseUrl + 'index.html', fetchOpts);
        var configFetch = (pf && pf.baseUrl === baseUrl && pf.config) ? pf.config : fetch(baseUrl + 'config.json', fetchOpts);
        ...
        if (indexCacheHit) {
            ...
            indexFetch
                ...
                .then(function (txt) {
                    if (typeof txt === 'string' && txt.length && txt !== cachedIndex.body) {
                        writeWebIndexCache(serverUrl, txt);
                    }
                    if (revalStart) {
                        try { window.__shellIndexCacheSavedMs = Date.now() - revalStart; } catch (_) {}
                    }
                })
                .catch(function () {});
            configFetch
                .then(function (r) { return r && r.ok ? r.text() : null; })
                .then(function (txt) {
                    if (typeof txt === 'string' && txt.length && txt !== cachedConfig.body) {
        ...
        var prefetchedBundle = (pf && pf.baseUrl === baseUrl && pf.bundle && pf.bundleUrl)
        ...
        try {
            window.__shellPluginPrefetch = (pf && pf.baseUrl === baseUrl && pf.plugins) ? pf.plugins : null;
        } catch (_) {}
        // Release prefetch refs so the connect-screen retry path issues fresh
        // fetches against the new server.
        try { window.__shellPrefetch = null; } catch (_) {}
        var indexPromise = indexCacheHit
            ? Promise.resolve(cachedIndex.body)
            : indexFetch
                .then(function (r) {
                    if (!r.ok) throw new Error('Failed to fetch web client (HTTP ' + r.status + ')');
                    return r.text();
                })
                .then(function (txt) {
        ...
        var configPromise = indexCacheHit
            ...
            : configFetch
                .then(function (r) {
                    if (!r.ok) throw new Error('Failed to fetch web config (HTTP ' + r.status + ')');
                    return r.text();
                })
                .then(function (txt) {
                    if (cacheGateOn) writeWebConfigCache(serverUrl, txt);
                    try { return JSON.parse(txt); }
                    catch (e) { throw new Error('Failed to parse web config'); }
                });
        return Promise.all([indexPromise, configPromise])
            .then(function (results) {
                ...
                if (baseTag.nextSibling) doc.head.insertBefore(seedTag, baseTag.nextSibling);
