// JEL-85 connect-form parity test.
//
// Proves the TV bootstrap connect-form server probe (boot-shell.src.js
// `validateServer`) matches the browser/core path (shell-core
// `validateServerUrl`) on the two divergences found during JEL-30:
//
//   1. fetch timeout — core aborts the /System/Info/Public probe after 5s so a
//      black-hole host (firewalled IP, no RST) shows an error instead of
//      hanging forever. The TV path must reject within the same 5s budget.
//   2. Version required — core rejects a probe response unless BOTH `Id` and
//      `Version` are present. A pathological `{Id:...}`-only endpoint must fail
//      on TV too.
//
// The test extracts the *shipped* `validateServer` + `withBootTimeout` (and
// their timeout constants) straight from src/boot-shell.src.js and runs them in
// an isolated vm with an injected fetch + controllable timers — so it asserts
// the real code path, not a re-implementation.
//
// Run: node packages/shell-tizen-bootstrap/scripts/connect-form-parity.test.cjs
...
function extractFunction(src, name) {
  ...
  if (start === -1) fail(`could not locate function ${name} in boot-shell.src.js`);
  ...
  fail(`unterminated function ${name} in boot-shell.src.js`);
}

function extractConst(src, name) {
  const m = new RegExp("var " + name + "\\s*=\\s*([0-9]+)\\s*;").exec(src);
  if (!m) fail(`could not locate const ${name} in boot-shell.src.js`);
  return Number(m[1]);
}
...
// Parity assertion #0: the connect budget must equal core's 5000ms.
if (CONNECT_FETCH_TIMEOUT_MS !== 5000) {
  fail(
    `CONNECT_FETCH_TIMEOUT_MS is ${CONNECT_FETCH_TIMEOUT_MS}, expected 5000 ` +
      `(must mirror shell-core validateServerUrl's 5s budget)`,
  );
}
...
async function run() {
  ...
  sandbox.fetch = () =>
    new Promise(() => {
      neverResolved = true;
    }); // never settles — simulates a firewalled IP with no RST
  let timedOut = false;
