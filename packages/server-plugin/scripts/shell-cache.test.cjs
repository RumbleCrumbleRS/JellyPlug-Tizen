...
//
// JELA-688 inserted the gzip body as the middle argument; the raw-body/sha
// pairing this check exists for is unchanged. asset-gzip.test.cjs separately
// pins that each route also passes its OWN gzip body.
for (const [route, bytes, sha] of [
  ...
  assert.ok(
    new RegExp(
      `public IActionResult ${route}\\(\\)\\s*=>\\s*ContentAddressed\\(_drop\\.${bytes}, [^,]+, _drop\\.${sha}\\);`,
    ).test(ctrl),
...
// JELA-688 moved the File() call into the Tagged() helper so the identity and
// gzip representations can carry distinct tags; the sha is still what reaches
// entityTag, and ContentAddressed still returns through it on both paths.
const tagged = /private FileContentResult Tagged\([\s\S]*?\n\n/.exec(ctrl);
assert.ok(tagged, "Tagged helper not found");
assert.ok(
  /entityTag: new EntityTagHeaderValue\("\\""\s*\+\s*tag\s*\+\s*"\\""\)/.test(tagged[0]),
