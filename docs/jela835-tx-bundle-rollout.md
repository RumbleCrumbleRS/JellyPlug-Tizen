...
## 2. The JELA-824 kill switch — it _had_ landed, and the served bundle hid it
...
of the last config _save_, not the last config _write_. A deploy that restarts
Jellyfin can surface a seed that was invisible an hour earlier.** Re-check
`public.js` _after_ the restart, never only before.
...
- **A per-body cold boot measured 223,434 B — 15 B _over_ the AC2 gate.** The
...
- **Hash the served body, then hash it again after the restart.** The shell sha
  is the deploy proof; `public.js` needed a _second_ read for the same reason,
