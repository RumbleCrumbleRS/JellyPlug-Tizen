...
Split from **JELA-797**, which *refuted* the parent claim that localStorage size
...
defects, and at quota they degrade *plugin* behaviour, not rendering.
...
key with its *version-ish* tokens removed (`>=15`-digit ticks, dotted `a.b.c`,
...
- content-addressed `txc:` bodies are **not** dropped here on purpose: they are
  keyed by *source hash*, so a byte-identical body legitimately survives a
...
- `txSetStatic` tracks the key it wrote, and on quota now does
  ...
  having *actually evicted something*, so with the flag dark it collapses exactly
...
them against one shared store with a real UTF-16 quota. It pins the fix *and the
control arms that show the defect* — see the gating note below for why that is
not optional:

- (a) control: flag dark, a `?v=` bump leaves **both** generations, the store
  grows by a whole body, and the orphan is still readable.
- (a) fix: one generation survives, the old body is *gone*, the store grows by
...
1. **The control arm must SHOW the defect.** A version bump has to be *induced*,
...
4. A persistent profile hits a ~3.67M-char cliff of the shell's *own* transpile
