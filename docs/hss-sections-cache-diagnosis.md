...
cache entries were _"invalidated within 30 seconds despite `CacheTimeoutSeconds=86400`"_.
...
There _is_ a server-side cache: `UserSectionsDataCache.Cache`, a
...
The cache was only ever designed to serve _pagination within a single page load_: the
...
`UserSectionsData.LastAccessed` is _written_ at `:42` and **never read anywhere** — the
...
Measured with `hss-decay-probe.py` after the JELA-692 pre-flight gate returned
...
**There is no response cache.** The `immediate` column is a re-hit issued with _zero_ gap,
...
observation — its _attribution_ was wrong. It cannot be the HSS section cache, which is
...
- **JELA-693's AC1 — "cold hit under ~500 ms" — is unreachable by configuration**, and as
  ...
  build. No setting tunes a cache that is never read. What _is_ reachable is the goal behind
...
- **A fork is not required to understand or report this.** Whether one is required to _fix_
...
```bash
./preflight.sh                     # must print CLEAR
./hss-decay-probe.py    --user <userId> --out decay.json
./hss-pagehash-probe.py --user <userId> --out pagehash.json
```

The probe scripts are not repo files — JEL-141 keeps QA harnesses out of the
tracked tree, so they live in the measuring developer's workspace; their run
output is attached to the JELA-693 issue thread. `preflight.sh` is reproduced
in the appendix of `docs/perf-measurement-protocol.md`.
