...
2. It seeds `liteEnabled=0`. The fleet default is lite-ON, and the lite→SPA
   handoff is *precisely* what makes the channel post-handoff (JELA-802).
...
*after* the wire, so CDP frame receipt is gate-independent.
...
1. **In-boot positive control.** In the *same* armed boot, on the *same* socket,
   with the *same* scoring code, the rendered push produced 18–20 rebuild
